...
def process_entry(entry, out_file):
    ...
    if (len(rule) == 3 and rule[2] == '*' and hash_function != 'crc-16'):
      ...
      hash_name = compute_hash(name_components,'crc-16'.encode('utf-8')) 
...
def compute_hash(name_components,function):
    crc = crcmod.predefined.mkCrcFun(function.decode('utf-8'))
    return crc("".join(name_components).encode('utf-8'))

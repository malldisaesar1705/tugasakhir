...
struct metadata {
    ...
    @name(".cs_metadata")
    ingress_metadata_t    cs_metadata;
}
...
struct headers {
    ...
    @name(".ndnlpfragcount") 
    ndnlpfragcount_t     ndnlpfragcount;
}
...
@name(".cs_r") register<bit<8>, bit<16>>(32w65536) cs_r;
...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    // @name(".cleancsEntry") action cleanscEntry() {
    //     readPit();
    //     if()
    //     pit_r.write((bit<16>)16w0x0, (bit<8>)meta.name_metadata.name_hash);
    // }
    ...
    @name(".readCs") action readCs() {
        cs_r.read(meta.flow_metadata.isInCS, (bit<16>)meta.name_metadata.name_hash);
    }
    @name(".readCsEntry") action readCsEntry() {
        readCs();
    }
   @name(".updateCs_entry") action updateCs_entry() {
        meta.cs_metadata.tmp = (bit<8>)((bit<8>)meta.flow_metadata.isInCS & 8w0 << standard_metadata.ingress_port);
        meta.flow_metadata.isInCS = (bit<8>)((bit<8>)meta.flow_metadata.isInCS | 8w0 << standard_metadata.ingress_port);
        cs_r.write((bit<16>)meta.name_metadata.name_hash, (bit<8>)meta.flow_metadata.isInCS);
    }
    @name(".ubahport") action ubahport() {
    ...
    @name(".updatePit_entry") action updatePit_entry() {
        meta.pit_metadata.tmp = (bit<8>)((bit<8>)meta.flow_metadata.isInPIT & 8w0 << standard_metadata.ingress_port);
        meta.flow_metadata.isInPIT = (bit<8>)((bit<8>)meta.flow_metadata.isInPIT | 8w0 << standard_metadata.ingress_port);
    ...
    @name(".cs_table") table cs_table {
        actions = {
            readCsEntry;
        }
        size = 1;
    }
    @name(".updatecs_table") table updatecs_table {
        actions = {
            ubahport;
            updateCs_entry;
        }
    ...
    apply {
        ...
        if (meta.name_metadata.components != 8w0) {
            ...
            cs_table.apply();
            if (meta.flow_metadata.packetType == 8w0x6){
                if(meta.flow_metadata.isInCS == 8w0){
